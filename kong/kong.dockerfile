...
RUN apt update && apt install -y libssl-dev gcc git
...
ENV KONG_PLUGINS=bundled,oidc,kong-keycloak-acl,kong-kafka-log,kong-spec-expose,kong-upstream-jwt,kong-path-whitelist,kong-response-size-limiting
